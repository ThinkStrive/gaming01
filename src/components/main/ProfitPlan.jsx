...
import ProfitPlanChart from "../../components/reuse/charts/ProfitPlanChart";
...
const Planprofit = () => {
  ...
  const calculateUserProgress = (sessionCount) => {
    let data = [];
    let bankRoll = savedBankRoll;
    let profitGoalPercentage = savedProfitGoal;

    for (let i = 1; i <= sessionCount; i++) {
      const unitSize = (bankRoll * unit) / 100;
      const stopLossSize = (bankRoll * stopLoss) / 100;
      const todayGoal = ( bankRoll * profitGoalPercentage ) / 100;
      ...
      data.push({
        ...
        profitGoalPercentage,
        todayGoal:todayGoal.toFixed(2),
  ...
  const handleSaveClick = (index) => {
    ...
    for (let i = index + 1; i < updatedProgress.length; i++) {
      ...
      const profitGoalPer = updatedProgress[i].profitGoalPercentage;

      updatedProgress[i].todayGoal = ((bankRoll * profitGoalPer)/100).toFixed(2);
    ...
    <>
      ...
      <div className="my-5 mx-8">
        <div className="bg-purple-800 rounded-md">
          <h2 className="text-gray-200 text-base font-medium text-center p-1 md:p-2">
            Profit Plan
          </h2>
        </div>
        ...
        {!savedData ? (
          <div className="p-2 rounded-md mt-2">
            <h2 className="text-base font-medium text-center text-gray-200">
          ...
          <div className="mt-2">
            ...
            <div className="rounded-md min-w-[85vw] overflow-x-scroll">
              <table className="table-auto w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-purple-700 text-gray-300">
                ...
                <tbody>
                  {calculatedProgress.map((row, index) => (
                    <tr key={row.session} className={`${row.session % 2 === 1 ? "bg-purple-900":"bg-purple-800"} text-gray-300 cursor-pointer hover:bg-[#391f62]`}>
                      ...
                      <td className="border border-black px-1 py-0.5 text-center font-medium">
                        ...
                        min={1}
                        max={100}
                        className="px-2 py-1 bg-black text-gray-300 w-[60px] text-center rounded-md outline-none"
                      ...
                      <td className="border border-black px-4 py-2 text-center font-semibold">
                        <span className={row.profitPercentage > 0 ? "text-green-500" : row.profitPercentage < 0 ? "text-red-500" : "text-gray-300"}>{row.profitPercentage} %</span>
                      </td>
                      <td className="border border-black px-4 py-2 text-center font-semibold">
                        <span className={row.profitPercentage > 0 ? "text-green-500" : row.profitPercentage < 0 ? "text-red-500" : "text-gray-300"}>{savedCurrency} {row.actualProfit}</span>
                      </td>
                      <td className="border border-black px-4 py-2 text-center font-medium">
                        {editableRow === index ? (
                          <input
                            ...
                            className="px-2 py-1 bg-black text-gray-300 w-[100px] text-center rounded-md outline-none"
                          />
                        ) : (
