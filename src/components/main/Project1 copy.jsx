...
import Modal from "../resources/Modal.jsx";
...
const Project1 = ({ theme }) => {
 ...
 console.log("initial score",initialScoresST);
  ...
  useEffect(() => {
    ...
    if (landedNumbersDD.length >= 36) {
      ...
      landedNumbersDD.slice(0, 3).forEach((entry, index) => {
        ...
        // Update dozen scores
        if (entry.dozen) {
          newDozenScores[entry.dozen] += entry.number;
        }
        // Update column scores
        if (entry.col) {
          newColumnScores[entry.col] += entry.number;
        }
      ...
      landedNumbersDD.slice(0, 9).forEach((entry, index) => {
        ...
        // Update dozen scores
        if (entry.dozen) {
          newDozenScoresLong[entry.dozen] += entry.number;
        }
        // Update column scores
        if (entry.col) {
          newColumnScoresLong[entry.col] += entry.number;
        }
      ...
      console.log("Short Dozen Score",newDozenScores);
      console.log("Short Col Score",newColumnScores);
      console.log("Long Doz",newDozenScoresLong);
      console.log("Long Col",newColumnScoresLong);
  ...
  const handleClickNumber = (key,number,dozen,col) => {
    const {
      countUpdates,
      summaryUpdates,
      doubleStreetDataUpdates,
    ...
    setLandedNumbersDD((prev) => [{ key,number, dozen, col}, ...prev]);
  };
    ...
    <>
        ...
        >
          ...
          <div className="flex gap-2">
            <div className="bg-neutral-300 p-1 rounded-full">
              <div
                className="flex justify-center items-center bg-customBlack p-1.5 px-5 cursor-pointer rounded-full hover:bg-gray-500"
                onClick={() => setIsAlertAllowed(!isAlertAllowed)}
              >
                Alerts{" "}
                <span
            ...
            <div className="bg-neutral-300 p-1 rounded-full hover:bg-gray-400">
              <button
                className="bg-black text-white px-5 p-1.5 rounded-full btns max-sm:text-sm hover:bg-neonGreen"
                onClick={handleClickUndoButton}
              >
                Undo
              </button>
      ...
      <div className="px-4 main">
        ...
        >
          ...
          {/* <div className="md:w-full md:h-auto overflow-x-auto"> */}
          <div className="w-full flex justify-between">
            <div
              className="md:w-[35%] w-full md:max-sm:w-[50%] md:max-w-xs"
              style={{ maxHeight: "100%" }}
            >
              <div className="w-full h-[7%] flex">
                <div
          ...
          <div className="flex items-center justify-evenly w-[30vw] h-[5vh] border-2">
            ...
            <Modal
              ...
              title="Table"
            >
              <div className="flex justify-center items-center con-con4 p-2" >
                <table
                  border="1"
                  cellPadding="16"
                // className="table--1 min-w-max" // Ensures table width doesn’t shrink on overflow
                >
                  <thead>
